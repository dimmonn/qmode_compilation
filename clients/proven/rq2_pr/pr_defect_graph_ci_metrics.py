...
class DagToPrrPearsonRQ2:

    def __init__(self, project_owner: str):
        self.project_owner = project_owner

        data_handler = DataCacheHandler(
            '../../../queries/pull_defect_graph_ci_metrics.sql',
            f'../../../persistence/files/pull_defect_graph_ci_metrics_{project_owner}.parquet',
            project_owner
        )
...
def main():
    for owner in ["ansible", "facebook"]:
        print(f"\n=== RQ1 bug introduction pr correlations for owner = {owner} ===")
        model = DagToPrrPearsonRQ2(project_owner=owner)
        corr = model.run()
